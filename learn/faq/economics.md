...
The Rad is the native token of the Radix platform. Rads and XRD is the same token; XRD is simply the ticker symbol \(exchange listing name\) for the Rad.  
...
If buying Rads from another user, you simply place a buy order on the DEX for the number of Rads you want to buy and at what price.
...
In Stage 1, after Technical Go Live and before Economic Go Live, you will be able to purchase Rads from the system using USD- backed tokens from a group of Approved Minters. In Stage 2, after Economic Go Live, you will need Radix Index Tokens to buy Rads from the system.
...
If buying from the system, you must first purchase the component tokens from Approved Minters which are included in the XRI \(Radix Index Token\). You must then send these tokens in the correct proportion to XRI minting contract which will then mint XRI tokens. These tokens will then be transferred to your wallet.
...
{% hint style="info" %}
[Infographic on how to buy Rads.](https://uploads-ssl.webflow.com/5a5a128611f84b0001e85b9f/5c5315b1d8fe7db52ca1e65b_How%20to%20buy%20or%20sell%20Rads.png)
{% endhint %}
...
The Radix Team/Foundation will not create any asset-backed tokens. All Approved Minters will be separate entities from the Radix Foundation or renamed Radix DLT Ltd entity. A key reason for this is to reduce centralization within the system.

### **How many Approved Minters does the team estimate there will be?** 

Our hope is to have between 3-5 Approved Minters by Technical Go- Live; to aid easy on-boarding for users wishing to buy Rads and also to reduce any centralization risk by just relying on one1 Approved Minter.
...
Approved Minters are an essential element for the Radix Economic Model to function. Therefore, it is crucial for the team to ensure several minters on-boarded and ready for Technical Go Live.

If there are still no Approved Minters confirmed near the planned Technical GoLive date it might be delayed.
...
### **What happens if an Approved Minter gets hacked and they a\) create infinite Rads and; b\) remain undetected, distributing small amounts from time to time to themselves?**
...
When an event on the ledger triggers the need for the use of the Reserves, it is subject to the network consensus process, validated against the Economic Algorithm ruleset. This process ensures the Reserves can operate autonomously, are audit-able and secure.
...
Instead, the expectation is that your wallet will make a claim automatically for you when it is open/running. The network will then check and return the correct balance, including any redistributed Rads.
